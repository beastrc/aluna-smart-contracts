...
const ERC20Mintable = contract.fromArtifact('Token');
const TokenVesting = contract.fromArtifact('AlunaTokenVesting');
...
describe('TokenVesting', function () {
  ...
  beforeEach(async function () {
    ...
    this.alunaTokenVesting = await TokenVesting.new();
  });
  ...
  it('reverts with a duration shorter than the cliff', async function () {
    ...
    await expectRevert(
      this.alunaTokenVesting.init(beneficiary, this.start, cliffDuration, duration, true, { from: owner }),
  ...
  it('reverts with a null beneficiary', async function () {
    await expectRevert(
      this.alunaTokenVesting.init(ZERO_ADDRESS, this.start, this.cliffDuration, this.duration, true, { from: owner }),
  ...
  it('reverts with a null duration', async function () {
    ...
    await expectRevert(
      this.alunaTokenVesting.init(beneficiary, this.start, 0, 0, true, { from: owner }), 'TokenVesting: duration is 0'
    );
  });
  ...
  it('reverts if the end time is in the past', async function () {
    ...
    await expectRevert(
      this.alunaTokenVesting.init(beneficiary, this.start, this.cliffDuration, this.duration, true, { from: owner }),
  ...
  context('once deployed', function () {
    beforeEach(async function () {
      ...
      await this.vesting.init( beneficiary, this.start, this.cliffDuration, this.duration, true, { from: owner });
    ...
    it('can be released after cliff', async function () {
      ...
      const event = await this.vesting.release(this.token.address);
      expectEvent( event, 'TokensReleased', {
    ...
    it('should be revoked by owner if revocable is set', async function () {
      const event = await this.vesting.revoke(this.token.address, { from: owner });
      expectEvent(event, 'TokenVestingRevoked', { token: this.token.address });
      (await this.vesting.revoked(this.token.address)).should.equal(true);
    });

    it('should fail to be revoked by owner if revocable not set', async function () {
      const vesting = await TokenVesting.new();
      await vesting.init(beneficiary, this.start, this.cliffDuration, this.duration, false, { from: owner });
